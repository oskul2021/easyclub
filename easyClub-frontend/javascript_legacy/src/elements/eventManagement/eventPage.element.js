...
export default function EventPage() {
	...
	function eventsDatesList(date, id) {
		//only display dates that do not repeat
		if (uniqeDateList.filter((e) => e.date === date).length > 0) {
			return <h2> {}</h2>;
		} else {
	...
	function generateCalendarEvents(event) {
		let eventData = {
			title: event.name,
			id: event.id,
	...
	if (listEvents) {
		...
		const unique = [ ...new Set(listEvents.map((item) => item.category)) ];
